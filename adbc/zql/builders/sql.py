...
class SQLBuilder(Builder):
    ...
    def normalize(self, query: Union[list, dict]):
        ...
        if "alter" in query:
            ...
            query = self.normalize_create(query)

        elif "drop" in query:
            query = self.normalize_drop(query)
    ...
    def normalize_create(self, clause: dict):
        """override"""
        return clause

    def normalize_drop(self, clause: dict):
        """override"""
        return clause
    ...
    def get_select_join(self, join, style, params, depth=0) -> str:
        ...
        if isinstance(join, dict):
            ...
            if type not in self.JOIN_TYPES:
                raise ValueError(f'select: invalid join type "{type}"')
            type = self.JOIN_TYPES[type].upper()
    ...
    def get_create_constraint(
        ...
        if type not in self.CONSTRAINT_TYPES:
            raise ValueError(f'constraint "{name}" type invalid: "{type}"')

        type = self.CONSTRAINT_TYPES[type]
        check = ""
